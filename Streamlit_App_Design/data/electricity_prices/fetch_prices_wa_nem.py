"""
fetch_prices_wa_nem.py — Fetch 7-day electricity prices from OpenElectricity API.

Renamed from '3days_Prices_WA&NEM.py' so it can be imported by other Python
modules (the '&' character in the old name made import impossible).

When run directly:   python fetch_prices_wa_nem.py
    → fetches data and writes to '7 days elec price.csv' in this directory

When imported:        from data.electricity_prices.fetch_prices_wa_nem import retrieve_all_data
